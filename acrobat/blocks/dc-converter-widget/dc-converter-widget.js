...
export default function init(element) {
  const innerInit = function(DC_WIDGET_VERSION) {
    console.log('in inner init', DC_WIDGET_VERSION);
  ...
  fetch('https://main--dc--adobecom.hlx.page/drafts/sonja/book.json')
    .then((response) => response.json())
    .then((data) => {
      let DC_WIDGET_VERSION = data.data[0].version;
      console.log('dc widget version', DC_WIDGET_VERSION);
      innerInit(DC_WIDGET_VERSION);
    });
}
